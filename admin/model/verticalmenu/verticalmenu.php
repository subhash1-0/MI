...
class ModelVerticalmenuVerticalmenu extends Model { 
	 ...
	 */
	public function getChild( $id=null, $store_id ){
		$sql = ' SELECT m.*, md.title,md.description FROM ' . DB_PREFIX . 'verticalmenu m LEFT JOIN '
								.DB_PREFIX.'verticalmenu_description md ON m.verticalmenu_id=md.verticalmenu_id AND language_id='.(int)$this->config->get('config_language_id') ;
	 ...
	 */
	public function getTree( $id=null, $store_id ){
	...
	public function genTree( $parent, $level, $store_id = 0 ){
		if( $this->hasChild($parent) ){
			...
			foreach( $data as $menu ){
				$url  = $this->url->link('module/pavverticalmenu', 'id='.$menu['verticalmenu_id'].$store.'&token=' . $this->session->data['token'], 'SSL') ;

				$output .='<li id="list_'.$menu['verticalmenu_id'].'">
				...
				if($menu['verticalmenu_id'] > 1) {
					$output .= $this->genTree( $menu['verticalmenu_id'], $level+1 );
				}
				$output .= '</li>';
			}	
	 ...
	 */
	public function getDropdown( $id=null, $selected=1, $store_id ){
		$this->children = array();
		$childs = $this->getChild( $id, $store_id );
		foreach($childs as $child ){
			$this->children[$child['parent_id']][] = $child;	
		...
		foreach ($categories as &$category){
			...
			if($this->checkExitItemMenu($category, $store_id) == 0){
				if((int)$category['parent_id'] > 0){
					...
					if($query1->num_rows){
						$megamenu_parent_id = (int)$query1->row['verticalmenu_id'];
					}
				} else {
	...
	public function insertCategory($category = array(), $megamenu_parent_id, $store_id = 0){
		...
		$data['verticalmenu']['parent_id'] = $megamenu_parent_id;
	...
	public function install(){
		...
		if( file_exists($file) ){ 
			...
			require_once( (DIR_APPLICATION).'model/sample/module.php' );
	 	    ...
	 	    $result = $sample->installSampleQuery( $this->config->get('config_template'),'pavverticalmenu', true );
	 	    $result = $sample->installSample( $this->config->get('config_template'),'pavverticalmenu', true );
		} else { 
